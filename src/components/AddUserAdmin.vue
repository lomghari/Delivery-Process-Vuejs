...
<v-container class="pa-6">
  ...
  <v-form class="mt-16">
     ...
     <v-select :items="roles" label="Role" prepend-icon="portrait" v-model="Role"></v-select>
     <v-divider class="mt-5 mb-8"></v-divider>
     <v-btn depressed class="primary d-block" large width="100%" :loading="btnLoader" @click.prevent="Singup">
        <span>Singup</span>
     </v-btn>
  </v-form>
</v-container>
...
export default {
  data () {
    return {
      showPassword: false,
      roles: ['Admin', 'Operateur', 'Manager', 'Rider']
    }
  },
  computed: {
    Password: {
